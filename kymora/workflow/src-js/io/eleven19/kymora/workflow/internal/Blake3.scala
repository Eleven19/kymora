...
/** Scala.js cross-platform hash fallback.
  *
  * '''Not''' a real BLAKE3. To avoid a Scala.js BLAKE3 port (or a Web-Crypto
  * async dependency) this ships a deterministic '''FNV-1a 64-bit''' digest
  * that compiles on every Scala 3 backend with zero platform deps.
  *
