...
/** Errors raised by the cache + manifest storage layer.
  *
  * Sealed + `Schema`-derived so storage failures can be persisted alongside results (e.g. in negative-cache entries)
  * and round-tripped through the observability bus.
  */
...
object StoreError:
    ...
    /** Lift an arbitrary `Throwable` from an I/O boundary into a `StoreError`, preferring `getMessage` over the class
      * name when available.
      */
