...
/** Structured parse failures for workflow value types.
  *
  * Extends `RuntimeException` so the same value can be threaded through Abort/Result channels (errors-as-values) AND
  * thrown from total encoders/decoders (e.g. kyo-schema's `readFn: Reader => A`, which has no Result channel of its
  * own).
  *
  * Each variant supplies a short `description` (the headline summary) and an optional `detailedDescription` carrying
  * the expected form, known-good values, or other repair hints. `getMessage` concatenates the two so a bare throw
  * surfaces both halves; pattern-match consumers can read either field directly without parsing the message.
  *
  * `detailedDescription` is a constructor field on each variant — the companion defines a sensible default so most call
  * sites just say `MalformedTaskVersion("x.y")`, and a use-site that has extra context can pass its own `Maybe[String]`
  * to override.
  */
