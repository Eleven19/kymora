...
/** The key the engine uses to look up a manifest. Mill-style: dotted [[TaskId]] -> slash-separated path under the cache
  * root.
  *
...
/** A read manifest: encoded bytes plus the actual file path on disk (useful for diagnostic errors).
  */
final case class StoredManifest(bytes: Chunk[Byte], path: VPath)

/** Pluggable cache store.
  *
  *   - `openWorkspace` returns a fresh `.dest.tmp/` workspace path for a `Task.Cached` body; the `Scope` finalizer
  *     cleans it up on failure, and the engine atomically renames it to `.dest/` on success.
  *   - `openPersistentWorkspace` returns the in-place `.dest/` for a `Task.Persistent` body, taking a per-key advisory
  *     lock for the `Scope`. The workspace is intentionally retained on body failure.
  */
trait CacheStore:
