...
/** Default cache store: Mill-style flat layout under a VFS directory.
  ...
  *   - `<root>/<id-segments>.dest/` — `Task.Persistent` working dir, retained across successful and failed runs and
  *     held under `PersistentMutex` for the scope of one body invocation.
  *   - `<root>/<id-segments>.dest.tmp/` — `Task.Cached` staging dir, atomically renamed to `.dest/` on success and
  *     removed on Scope exit otherwise.
  *
  * `read` / `write` use atomic rename through a sibling `.tmp` file. `purge` drops the entire root, `remove` drops a
  * single manifest + its `.dest/`, and `list` walks the root reverse-engineering keys from `.json` paths.
  */
object VfsDirStore:
    ...
    /** Builds the absolute manifest path for `key` under `root`.
      *
      * The leaf segment of the key gets a `.json` suffix; intermediate segments become directory components (Mill-style
      * flat layout).
      */
    ...
    /** Builds the path to the `<key>.dest.tmp/` staging workspace.
      *
      * This sibling-of-`.dest` location is where a `Task.Cached` body writes its outputs. On success the engine
      * atomically renames it to `.dest/`; on failure the surrounding `Scope` finalizer removes it.
      */
    ...
    /** Converts an absolute path under `root` ending in `.json` back into a [[CacheKey]]. Returns `Absent` for
      * non-`.json` entries (directories, sidecar files, etc.).
      */
    ...
    private object PersistentLocks:
        private val mutex = new PersistentMutex

        private def name(root: VPath, key: CacheKey)(using Frame): String =
            s"${root.show}:${key.value}"

        def acquire(root: VPath, key: CacheKey)(using Frame): Unit < Async =
            mutex.acquire(name(root, key))

        def release(root: VPath, key: CacheKey)(using Frame): Unit < Sync =
            mutex.release(name(root, key))
    end PersistentLocks

    final private class Impl(root: VPath, vfs: Vfs.Backend) extends CacheStore:
        ...
        /** Removes the entire cache root subtree.
          *
          * No-op if the root does not exist. Implemented as a recursive delete via [[Vfs.removeAll]].
          */
        ...
        /** Walks `root` and returns every known [[CacheKey]] whose path begins with `prefix`.
          *
          * Walk-based (no on-disk index): each `.json` file under `root` is reverse-engineered into a `CacheKey` by
          * stripping the `.json` suffix and computing the path relative to `root`.
          */
                ...
                vfs.exists(root)
                    .map: rootExists =>
                        if !rootExists then Chunk.empty[CacheKey]
                            ...
                            Scope
                                .run(vfs.walk(root).run)
                                .map: entries =>
                                    entries.flatMap(toCacheKey(root, _)).filter(_.value.startsWith(prefix))

        /** Returns a fresh `<root>/<key>.dest.tmp/` staging path for a `Task.Cached` body.
          *
          * The returned path is registered with the current `Scope` so that an unsealed workspace (i.e. exception or
          * failure before the engine renames it to `.dest/`) is removed on scope exit. Sealing is the engine's
        ...
        def openWorkspace(key: CacheKey): VPath < (Async & Scope & Abort[StoreError]) =
            ...
            Abort.recover[VfsError](e => Abort.fail(StoreError.fromThrowable(p.show, e))):
                for
                    _ <- vfs.removeAll(p)
                    _ <- vfs.mkDirs(p)
                    _ <- Scope.ensure(Abort.run(vfs.removeAll(p))).unit
                yield p

        /** Returns the in-place `<root>/<key>.dest/` directory for a `Task.Persistent` body.
          *
          * Acquires a process-wide advisory lock keyed by `root` + `key.value` and releases it on `Scope` exit. The
          * directory itself is NOT removed on success or failure — persistence is the entire point of this workspace.
          */
        def openPersistentWorkspace(key: CacheKey): VPath < (Async & Scope & Abort[StoreError]) =
            val p = destPath(root, key)
            Abort.recover[VfsError](e => Abort.fail(StoreError.fromThrowable(p.show, e))):
                for
                    _      <- PersistentLocks.acquire(root, key)
                    _      <- Scope.ensure(PersistentLocks.release(root, key)).unit
                    exists <- vfs.exists(p)
                    _      <- if exists then (): Unit < (Sync & Abort[VfsError]) else vfs.mkDirs(p)
                yield p
    end Impl
end VfsDirStore
