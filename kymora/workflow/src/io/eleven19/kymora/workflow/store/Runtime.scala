...
/** Runtime metadata embedded in a [[TaskRecord]] manifest.
  ...
  *
  * Fields are all simple primitives, so `Schema` derivation works without additional opaque-type givens.
  */
final case class Runtime(os: String, arch: String, jvm: Maybe[String]) derives Schema, CanEqual
