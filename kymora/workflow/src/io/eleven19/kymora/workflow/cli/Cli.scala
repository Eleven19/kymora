...
object Cli:
  ...
  /** Parses `tokens` into a `P` via the implicit [[caseapp.core.parser.Parser]]
    * and runs `task(p)` through [[Workflow.run]].
    *
    * The Abort channel carries both [[WorkflowError]] (task execution) and
    * [[CliParseError]] (token parsing) — callers can recover from each
    * independently.
    */
  ...
  /** Parses `tokens` into a `P`, dispatches to the matching sub-command, and
    * runs it through [[Workflow.run]].
    *
    * The first token selects the command by `name`; remaining tokens are
    * parsed via that command's `Parser`. Unknown command names surface as
    * [[CliParseError.UnknownCommand]] with the registered command names
    * suggested via [[TaskId]] for consistency with engine error reporting.
    */
  def runCommands[A](
