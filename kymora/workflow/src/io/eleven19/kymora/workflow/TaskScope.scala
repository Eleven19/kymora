...
/** Ambient definition scope used to prefix task ids.
  *
  * Scopes let libraries define local task names while callers compose them into larger graphs.
  * `Workflow.scope("module") { ... }` installs a scope for the tasks defined in its body.
  */
