...
final private[workflow] case class CacheLayout(root: VPath):
    ...
    def openPersistentWorkspace(
        vfs: Vfs.Backend,
        key: CacheKey
    )(using Frame): VPath < (Async & Scope & Abort[WorkflowError]) =
        val path     = destPath(key)
        val lockName = s"${root.show}:${key.value}"
        recover(path):
            for
                _      <- PersistentWorkspaceLocks.acquire(lockName)
                _      <- Scope.ensure(PersistentWorkspaceLocks.release(lockName)).unit
...
private object PersistentWorkspaceLocks:
