...
private[workflow] object Hashing:
    ...
    /** A dep fingerprint pair: (dep's TaskId, dep's valueHash) — used by inputsHash so a downstream task invalidates
      * when an upstream's value changes.
      */
    ...
    /** inputsHash(task) = blake3( "kymora-workflow/v1" || task.id || bodyHash || join("|",
      * deps.sortedBy(_.id.value).map(d => d.id || ":" || d.fingerprint)) || maybe(paramHash) )
      *
      * Deps are sorted by id so declaration-order permutations don't change the hash. The body executes deps in
      * declaration order; this is separate.
      *
      * `paramHash` is folded in only when present, so unparameterized tasks produce the same fingerprint they did
      * before parameterization landed.
      */
    def inputsHash(
        taskId: TaskId,
