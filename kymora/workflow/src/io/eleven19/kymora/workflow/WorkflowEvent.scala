...
/** Structured event emitted by the engine for observability.
  *
  * Schema derivation is intentionally NOT attached to the sealed trait. If a downstream consumer needs a
  * `Schema[WorkflowEvent]`, provide an explicit event schema at that boundary so opaque identifiers and fingerprints
  * keep their stable encodings.
  */
