...
/** Produces stable fingerprints for values that influence workflow cache keys.
  *
  * Most case classes can use the default `Schema`-based derivation. Define a custom instance when the cache key should
  * ignore, normalize, or otherwise canonicalize parts of a value.
  */
trait Hashable[A]:
...
object Hashable:
    ...
    /** Default: encode via Schema using the canonical wire format (independent of Workflow.Runtime.codec) and blake3
      * the bytes. See spec §3.2.
      */
