...
/** Semantic version for a task body.
  *
  * Bump the version when a task's implementation changes in a way that should invalidate previous cached results even
  * if its dependencies did not change.
  */
        ...
        s.split('.') match
            case Array(a, b, c) =>
                (a.toIntOption, b.toIntOption, c.toIntOption) match
                    case (Some(majorN), Some(minorN), Some(patchN)) if majorN >= 0 && minorN >= 0 && patchN >= 0 =>
