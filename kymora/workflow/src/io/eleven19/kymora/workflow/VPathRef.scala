...
/** A Mill-style `PathRef` analogue: a virtual filesystem path bundled with the
  ...
  */
final case class VPathRef(path: VPath, fingerprint: Fingerprint, quick: Boolean = false)
  derives CanEqual

object VPathRef:
  private val Prefix = "vref"

  /** Builds a content-hashed `VPathRef` by reading the file at `path` and
  ...
  /** Builds a quick-hashed `VPathRef` from `size|mtime` rather than the full
