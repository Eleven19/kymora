...
/** Kyo effect for executing workflow tasks.
  *
  * `Workflow` is the public execution effect for `kymora-workflow`. Programs
  * describe task execution with [[Workflow.run]], [[Workflow.runAll]], or the
  * shorthand `task()`. Callers provide a [[Workflow.Runtime]] with
  * [[Workflow.handle]].
  *
  * {{{
...
object Workflow:
  ...
  /** Concrete runtime dependencies used to handle the [[Workflow]] effect.
    *
    * `config` is pure run policy. `vfs`, `cacheRoot`, `observer`, and `codec`
    * are runtime capabilities selected by the host application. Use
    * `Runtime(vfs)` when you already have a backend, or [[Runtime.default]]
    * for an in-memory runtime with the default cache root and silent observer.
    */
  ...
  /** Ambient workflow context.
    *
    * User code usually accesses this through [[runtime]], [[context]], and
    * [[dest]] rather than constructing it directly.
    */
  final case class Context(runtime: Runtime, task: Maybe[TaskContext])

  /** Pure run-level configuration.
    *
    * This type intentionally contains no services or codecs. Operational
    * dependencies live on [[Runtime]].
    */
  ...
  final class WorkflowException(val error: WorkflowError)
  ...
  def context(using Frame): TaskContext < (Workflow & Abort[WorkflowError]) =
    get.flatMap {
      ...
      case _                        =>
        Abort.fail(WorkflowError.TaskFailed(TaskId.unsafe("workflow"), "TaskContext is only available inside a task"))
    }
